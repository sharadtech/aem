...
@ServiceDescription("SHARADTECH | Google 2F Authentication Handler")
@Designate(ocd = TwoFactorAuthHandler.class)

public class OTPBasedAuthenticationHandler extends DefaultAuthenticationFeedbackHandler implements AuthenticationHandler, AuthenticationFeedbackHandler {
	...
	public AuthenticationInfo extractCredentials(HttpServletRequest request, HttpServletResponse response) {
		...
		if ((REQUEST_METHOD.equals(request.getMethod())) && (request.getRequestURI().endsWith(REQUEST_URL_SUFFIX)) && (request.getParameter(USER_NAME) != null)) {
			try {
				...
				if (session != null) {
					boolean is2StepAuthEnabled = check2StepAuthPreference(request.getParameter(USER_NAME), session);
					...
					if (is2StepAuthEnabled) {
						...
						return createAuthenticationInfo(request, response, request.getParameter(USER_NAME));
	...
	private boolean check2StepAuthPreference(String userId, Session session1) throws AccessDeniedException, RepositoryException, LoginException {
	...
	private AuthenticationInfo createAuthenticationInfo(HttpServletRequest request, HttpServletResponse response, String userId) throws RepositoryException {
	...
	public boolean requestCredentials(HttpServletRequest request, HttpServletResponse arg1) throws IOException {
		...
		try {
			String path = rewrite(request, this.resourceResolverFactory.getResourceResolver(null), config.loginPageValue());
	...
	private String checkOrCreateSecurityKey(String userId, Session session) throws AccessDeniedException, UnsupportedRepositoryOperationException, RepositoryException {
		...
		try {
			...
			if (node.hasProperty(SECRET_KEY)) {
				Property references = node.getProperty(SECRET_KEY);
	...
	private AuthenticationInfo twoFactorAuthentication(boolean is2StepAuthEnabled, HttpServletRequest request, HttpServletResponse response, Session session) throws RepositoryException {

		String needNewKey = checkOrCreateSecurityKey(request.getParameter(USER_NAME), session);
		if ((needNewKey != null) && (needNewKey.length() > 0)) {
			...
			if (request.getParameter("j_otpcode").length() <= 0) {
				log.info("2FA enabled but no OTP provided by user: {}, logout the session.", request.getParameter(USER_NAME));
			...
			if (!isSameCode) {
				if (checkCode(response, needNewKey, Long.parseLong(request.getParameter(OTPCODE)), new Date().getTime(), 23)) {
					return createAuthenticationInfo(request, response, request.getParameter(USER_NAME));
				}
				request.setAttribute("j_reason", "invalid_otp");
				return AuthenticationInfo.FAIL_AUTH;
